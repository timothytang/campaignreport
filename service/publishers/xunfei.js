  ...
  request.get({url:loginUrl, jar: j}, function(err,httpResponse,body){
      if (!err) {
        ...
        async.whilst( // TODO: change to each list
          ...
          function (callback) {
              ...
              request.post({url:reportURL, jar:j,form:{start_dt: startDateStr, end_dt:startDateStr}}, 
                   function(err,httpResponse,body){
                        if (err) {
                          console.error('Error: failed to get Xunfei data on day: ' + startDateStr);
                          callback(err, 1);
                        } else {
                          $ = cheerio.load(body);
          ...
          function (err, result) {
            if (err) {
              console.error('Error: failed to get report from Xunfei.');
            }
            externalCallback(allDateDetails);
          }
        );
      } else {
        console.error('Error: failed to login to xunfei.');
        externalCallback(allDateDetails);// directly return if error
      }
      console.log('Xunfei data processing is done.');
  });
